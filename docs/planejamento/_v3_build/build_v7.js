// Gera 08_ROTEIRO_GRAVACAO_VIDEO_V7.pdf com prints reais capturados ao vivo
...
parts.push(warn(
  ...
  "Esta revisão corrigiu 4 pontos que só apareceram testando ao vivo (2 herdados da V4, 2 novos — ver abaixo): (a) o print do erro #5005 exige dois cliques a mais do que o documentado antes — abrir a aba <b>Trace</b> da mensagem, clicar em <b>View Full Trace</b> e depois no losango vermelho de erro; " +
  ...
  "(c) <b>[V6]</b> um bug de unidade real foi corrigido no código: a janela de \"erro recente\" que decide o status <code>degraded</code> (<code>Guardian.Monitor.StatusCollector::ErrorWindowSeconds</code>) estava em 900s (15 minutos) — o comentário original dizia que devia bater com o <code>FailureTimeout</code> real do framework, que é <b>15 segundos</b>, não 15 minutos. Corrigido para <code>15</code> e confirmado ao vivo: agora, depois de corrigir a permissão e mandar tráfego novo, os hosts voltam a <code>healthy</code> em ~15-20 segundos; " +
  "(d) <b>[novo nesta versão]</b> a mensagem de recuperação da seção 6.2 estava com severidade <code>LOW</code>, que a regra <code>RouteBySeverity</code> nunca manda para <code>out_priority</code> (o destino que quebramos e consertamos) — ela ia para <code>out</code>, que nunca esteve bloqueado, então não provava nada sobre a correção. Corrigido para <code>CRITICAL</code> (mesma severidade do <code>INC-003</code>) e confirmado ao vivo: agora o arquivo aparece de verdade em <code>out_priority</code> depois do <code>chmod 755</code>."
));
...
<tr><td><b>Healthy / Degraded / Unavailable</b></td><td>Saudável (verde), degradado (amarelo: funciona com problema) e indisponível (vermelho: fora do ar). Um host fica <code>degraded</code> enquanto houver fila pendente ou um erro nos últimos <b>15 segundos</b> — a mesma janela de tempo que o framework usa para desistir de uma tentativa (<code>FailureTimeout</code>). Corrigido um bug de unidade nesta revisão: antes essa janela estava em 15 <i>minutos</i> por engano.</td></tr>
...
parts.push(step("6.2", "Mostrar que o destino consertado realmente aceita escrita de novo",
  ...
  cmd(`docker exec -i iris-guardian sh -c 'F=/durable/guardian/in/demo4.txt; echo "INC-004|IRIS.Service.PaymentGateway|CRITICAL|Fluxo critico apos recuperacao do destino" > $F && chown irisowner:irisowner $F'`),
  ...
  where("G", "Clique em <b>Monitor</b> no menu lateral esquerdo. Espere cerca de 15-20 segundos se ainda aparecer algum host <code>degraded</code>."),
  simple("O <code>INC-004</code> já foi gravado com sucesso na hora — não precisa esperar nada para isso. A espera de 15-20s é só para a janela de \"erro recente\" (<code>degraded</code> = fila pendente ou erro nos últimos 15 segundos, a mesma janela que o framework usa para desistir de uma tentativa) esgotar o registro do <code>INC-003</code>, que falhou antes. Passado esse tempo sem nenhum erro novo, os quatro hosts voltam a <code>healthy</code> sozinhos."),
  say(["O sistema aceita e processa o tráfego crítico imediatamente, sem reiniciar nada — mandei de propósito a mesma severidade que falhou antes, para provar que o destino realmente voltou a aceitar escrita, não só que o tráfego de baixa prioridade continua passando. O indicador de saúde acompanha isso de perto: assim que passam os 15 segundos da janela de detecção de erro sem nenhum problema novo, os quatro hosts voltam a healthy sozinhos."]),
...
const doc = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Roteiro de Gravação V7</title><style>${CSS}</style></head><body>${parts.join("")}</body></html>`;
...
(async () => {
  ...
  await pg.pdf({
    ...
    footerTemplate: '<div style="font-size:8px;color:#777;width:100%;text-align:center;">IRIS Production Guardian · Roteiro de Gravação V7 · <span class="pageNumber"></span>/<span class="totalPages"></span></div>',
