// Gera 08_ROTEIRO_GRAVACAO_VIDEO_V6.pdf com prints reais capturados ao vivo
...
parts.push(warn(
  ...
  "Esta revisão corrigiu 3 pontos que só apareceram testando ao vivo (2 herdados da V4, 1 novo — ver abaixo): (a) o print do erro #5005 exige dois cliques a mais do que o documentado antes — abrir a aba <b>Trace</b> da mensagem, clicar em <b>View Full Trace</b> e depois no losango vermelho de erro; " +
...
parts.push(step("6.2", "Mostrar que tráfego novo passa",
  ...
  where("G", "Espere cerca de 15-20 segundos e clique em <b>Monitor</b> no menu lateral esquerdo."),
  simple("<code>degraded</code> significa \"fila pendente ou erro nos últimos 15 segundos\" — a mesma janela de tempo que o framework usa para desistir de uma tentativa (<code>FailureTimeout</code>). Passados esses ~15-20 segundos sem nenhum erro novo, os quatro hosts voltam a <code>healthy</code> sozinhos."),
  say(["O sistema aceita e processa o tráfego novo imediatamente, sem reiniciar nada, e o indicador de saúde acompanha isso de perto: assim que passam os 15 segundos da janela de detecção de erro sem nenhum problema novo, os quatro hosts voltam a healthy sozinhos."]),
...
const doc = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Roteiro de Gravação V6</title><style>${CSS}</style></head><body>${parts.join("")}</body></html>`;
...
(async () => {
  ...
  await pg.pdf({
    ...
    footerTemplate: '<div style="font-size:8px;color:#777;width:100%;text-align:center;">IRIS Production Guardian · Roteiro de Gravação V6 · <span class="pageNumber"></span>/<span class="totalPages"></span></div>',
