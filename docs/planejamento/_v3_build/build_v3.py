#!/usr/bin/env python3
# Gera 08_ROTEIRO_GRAVACAO_VIDEO_V3.pdf a partir de conteudo escrito aqui.
# Prints reais existentes sao embutidos; os demais viram quadros "PRINT PENDENTE".
import base64, html, os, sys
from playwright.sync_api import sync_playwright

BASE = "/mnt/user-data/uploads/IRIS Production Guardian/docs/planejamento/screenshots/"
REAL = {
    "R1": BASE + "rule_editor_path.png",
    "R2": BASE + "rule_editor_loaded.png",
}
HERE = os.path.dirname(os.path.abspath(__file__))
# Prints de REFERENCIA (12/09/2026): anotados com bolinhas vermelhas, mas de versao anterior da UI.
REFS = {
    "P05": ("ann_P05.png", "Monitor. Print antigo (12/09): idioma PT e hosts em Unavailable. Na gravação: idioma EN e todos healthy. Os cliques são os mesmos."),
    "P15": ("ann_P15.png", "Investigator. Print antigo (12/09): idioma PT (Componente/Janela/Investigar viram Component/Window/Investigate em EN). Os cliques são os mesmos."),
    "P17": ("ann_P17.png", "Resultado da análise. Print antigo (12/09): a captura termina antes dos blocos Hipótese e Lacunas, que ficam abaixo de Observações (role a página)."),
    "P18": ("ann_P18.png", "RAG Assistant (Gemini). Print antigo (12/09): mostra um erro de cota do Gemini; serve só para localizar menu, caixa de pergunta e botão."),
    "P20": ("ann_P20.png", "RAG Assistant (Groq). Print antigo (12/09): o menu se chamava “RAG Assistant (Novo Modelo)”; hoje é “RAG Assistant (Groq)”. Os cliques são os mesmos."),
    "R3": ("ann_R3.png", "Atalho: na interface nova do Interoperability, a barra de abas no topo tem Production Configuration (1) e Message Viewer (2). Print antigo (12/09), porta 52773 e idioma PT; barra de favoritos removida do recorte."),
}
REFUSED = []
OUT_HTML = sys.argv[1]
OUT_PDF = sys.argv[2]
...
PENDING = []  # (id, titulo, como capturar)

def shot(pid, title, where, mark, note=""):
    """Quadro de print. Se houver imagem real, embute; senao mostra pendencia."""
    if pid in REFS:
        fn, note2 = REFS[pid]
        REFUSED.append((pid, title, note2))
        return (f'<div class="shot"><img src="{img64(os.path.join(HERE, fn))}">'
                f'<div class="cap ref">PRINT {pid} — {e(title)}. <b>REFERÊNCIA:</b> {e(note2)}</div></div>')
    if pid in REAL:
        body = f'<img src="{img64(REAL[pid])}">'
        cap = f'<div class="cap real">PRINT {pid} — {e(title)}. {e(note)}</div>'
        return f'<div class="shot">{body}{cap}</div>'
    PENDING.append((pid, title, where, mark))
    return (f'<div class="shot pend"><div class="ph"><div class="phid">PRINT PENDENTE {pid}</div>'
            f'<div class="phtitle">{e(title)}</div>'
            f'<div class="phrow"><b>Tela:</b> {e(where)}</div>'
            f'<div class="phrow"><b>Bolinha vermelha em:</b> {e(mark)}</div></div></div>')

def cmd(text, label="Comando único (uma linha só)"):
    return f'<div class="cmdlabel">{e(label)}</div><pre>{e(text)}</pre>'

def say(text_paragraphs):
    ps = "".join(f"<p>{e(p)}</p>" for p in text_paragraphs)
    return f'<div class="say"><div class="saylabel">O QUE EU DIGO (primeira pessoa)</div>{ps}</div>'

def where(kind, text):
    tag = {"T": ("TERMINAL", "t"), "G": ("GUARDIAN", "g"), "P": ("PORTAL IRIS", "p")}[kind]
    return f'<div class="where"><span class="tag {tag[1]}">{tag[0]}</span> {text}</div>'

def simple(text):
    return f'<div class="simple"><b>Em palavras simples:</b> {text}</div>'

def warn(text):
    return f'<div class="warn">{text}</div>'

def step(n, title, *blocks):
    return f'<div class="step"><div class="stephead"><span class="num">{n}</span>{e(title)}</div>{"".join(blocks)}</div>'

def section(title, sub=""):
    s = f'<div class="sub">{e(sub)}</div>' if sub else ""
    return f'<h2>{e(title)}</h2>{s}'
...
<div class="cover">
  ...
  <h1>Roteiro de Gravação V3</h1>
  <div class="tagline">Passo a passo com prints, feito para quem nunca trabalhou com interoperabilidade</div>
  <div class="meta">Versão de 19/09/2026 · Baseado no V2 (18/09/2026) · Login da aplicação e do Management Portal: <b>guardian / guardian</b></div>
</div>
''')

parts.append(warn(
    '<b>Estado deste arquivo (leia primeiro):</b> os prints marcados como <b>PRINT PENDENTE</b> ainda não foram capturados, '
    'porque a captura exige estar logado na aplicação e no Management Portal e rodar os comandos de terminal do seu Mac. '
    'Os prints do Business Rule Editor (R1 e R2) são atuais. Os marcados como <b>REFERÊNCIA</b> são de 12/09/2026, com bolinhas vermelhas nos cliques, mas de uma versão anterior da tela (idioma PT, estados antigos): servem para você achar onde clicar e serão trocados na versão final. '
    'A lista completa do que falta capturar está no <b>Anexo A</b> e também no arquivo <b>Ajustes.txt</b>, item 7.'))

# ------------------------------------------------------------------ COMO LER
parts.append(section("Como ler este documento"))
parts.append('''
...
<tr><td><div class="shoticon">Print</div></td><td>Imagem da tela onde você vai clicar. A <b>bolinha vermelha numerada</b> marca cada clique, na ordem.</td></tr>
</table>
...
# ------------------------------------------------------------------ GLOSSARIO
parts.append(section("Glossário para quem não conhece interoperabilidade",
...
<tr><td><b>Healthy / Degraded / Unavailable</b></td><td>Saudável (verde), degradado (amarelo: funciona com problema) e indisponível (vermelho: fora do ar).</td></tr>
...
# ------------------------------------------------------------------ MAPA DE JANELAS
parts.append(section("Monte sua mesa de gravação antes de começar",
...
parts.append(section("0. Antes de gravar", "Faça esta lista uma vez, na ordem. Nada aqui vai para o vídeo."))
s0 = []
s0.append(step("0.1", "Esconder tudo que é pessoal",
...
s0.append(step("0.2", "Ligar o container do IRIS",
...
s0.append(step("0.3", "Conferir se a Production está rodando",
...
s0.append(step("0.4", "Limpar as pastas de mensagens",
    where("T", "No Terminal 1:"),
...
s0.append(step("0.5", "Refresh forçado e idioma em inglês",
    where("G", "No navegador, abra <code>http://localhost:53773/csp/guardian/Guardian.UI.MonitorPage.cls</code> (é a <b>única</b> URL de página que você digita na gravação inteira). Faça login com <b>guardian / guardian</b> e dê um refresh forçado: <b>Cmd+Shift+R</b>. Um cache antigo pode mostrar a logo do loader gigante em vez do badge pequeno."),
    ...
    shot("P03", "Barra lateral do Guardian com o botão de idioma em EN",
         "Guardian, canto superior esquerdo da barra lateral", "(1) botão de idioma mostrando EN")))
s0.append(step("0.6", "Ensaio completo sem gravar",
...
# ------------------------------------------------------------------ SECAO 1
parts.append('<div class="pagebreak"></div>')
parts.append(section("1. Abertura (30 a 60 segundos)", "Tela parada, sem clicar em nada."))
parts.append(step("1.1", "Mostrar a aplicação e falar",
...
parts.append(step("1.2", "Slide de abertura (opcional)",
    "<p>Se quiser um slide em vez da aplicação parada, cole o prompt abaixo numa IA geradora de imagem (Gemini, ChatGPT, Ideogram). Ele usa as cores reais da aplicação. Já existe uma imagem gerada na pasta <code>output/imagegen</code> do projeto.</p>" +
...
# ------------------------------------------------------------------ SECAO 2
parts.append('<div class="pagebreak"></div>')
parts.append(section("2. Production saudável, com mensagens passando em tempo real (1 a 2 minutos)",
                     "Objetivo: mostrar como é o “normal”, para o público perceber a diferença quando algo quebrar."))
parts.append(step("2.1", "Mostrar o Monitor saudável",
...
parts.append(step("2.2", "Ligar o tráfego contínuo",
...
parts.append(step("2.3", "Mostrar que o tráfego é real",
...
parts.append(step("2.4", "Mostrar o tráfego por dentro, no painel de administração do IRIS",
    ...
    where("P", "No menu esquerdo, clique em <b>Interoperability</b> (1) → <b>View</b> → <b>Messages</b> (2)."),
    ...
    shot("R3", "Atalho pelas abas da interface nova do Interoperability", "", ""),
    ...
    warn("<b>A confirmar ao vivo:</b> o menu <b>Interoperability → Monitor</b> do Management Portal também tem telas de volume de mensagens. Confira o nome exato dos itens quando estiver logado e anote no Ajustes.txt antes de gerar a versão final.")))

# ------------------------------------------------------------------ SECAO 3
parts.append('<div class="pagebreak"></div>')
parts.append(section("3. Introduzir a falha controlada (2 a 3 minutos)",
                     "Vamos quebrar de verdade um dos destinos e ver o sistema reagir."))
parts.append(step("3.1", "Bloquear o destino prioritário",
    where("T", "No Terminal 1:"),
...
parts.append(step("3.2", "Disparar um incidente grave",
    ...
    simple("Este comando faz duas coisas encadeadas por <code>&amp;&amp;</code> dentro de <b>um único</b> comando: (a) cria o arquivo <code>demo3.txt</code> com o incidente <code>INC-003</code> de severidade <code>CRITICAL</code>; (b) entrega a posse do arquivo ao usuário do IRIS (<code>chown</code>). Você <b>não</b> precisa rodar cada parte separada."),
...
parts.append(step("3.3", "Ver a falha aparecer no Monitor (espere ao vivo)",
...
parts.append(step("3.4", "Provar que o erro é real: Message Viewer",
    where("P", "Volte à aba do Management Portal. Menu esquerdo: <b>Interoperability</b> → <b>View</b> → <b>Messages</b>. Clique no botão de atualizar a lista."),
    simple("A mensagem do <code>INC-003</code> vai aparecer com <b>Status = Error</b>. Clique nela para abrir os detalhes. Você vai ver o texto do erro: <code>ERROR #5005: Cannot open file '/durable/guardian/out_priority/incident_INC-003.txt'</code>, embrulhado em <code>&lt;Ens&gt;ErrFailureTimeout</code>. Traduzindo: o sistema operacional recusou a gravação."),
    shot("P13", "Message Viewer com a mensagem INC-003 em Status Error",
         "Management Portal → Interoperability → View → Messages", "(1) linha do INC-003 com status Error, (2) botão de atualizar"),
    shot("P14", "Detalhe da mensagem INC-003 mostrando o ERROR #5005",
         "Message Viewer, ao clicar na mensagem com erro", "(1) texto do erro #5005 / Cannot open file"),
    say(["Aqui a falha é real, não simulada na interface. O processo tentou escrever e o sistema operacional recusou."]),
    "<p><b>Alternativa por comando</b> (só se quiser mostrar em terminal; anote o <code>ID</code> da mensagem do INC-003, você vai precisar dele na seção 6). Cole as 6 linhas de uma vez:</p>",
    cmd("""docker exec -i iris-guardian iris session IRIS -U GUARDIAN <<'EOF'
...
# ------------------------------------------------------------------ SECAO 4
parts.append('<div class="pagebreak"></div>')
parts.append(section("4. AI Incident Investigator (2 a 3 minutos)",
                     "A IA olha os dados reais da falha e monta uma investigação. Só lê, nunca corrige."))
parts.append(step("4.1", "Abrir o Investigator e escolher o problema",
...
parts.append(step("4.2", "Rodar a investigação",
...
parts.append(step("4.3", "Ler o resultado para a câmera",
    "<p>Leia em voz alta as três partes, uma a uma. É o ponto mais importante do vídeo:</p>"
    "<ul><li><b>Observação</b>: o que os dados mostram.</li><li><b>Hipótese</b>: a causa mais provável.</li><li><b>Lacunas</b>: o que <i>não</i> dá para afirmar com os dados disponíveis.</li></ul>",
    ...
    say(["Repare que a IA separa o que os dados mostram, que é a observação, do que ela supõe, que é a hipótese, e do que ela não consegue afirmar, que são as lacunas. Ela nunca corrige nada sozinha. A decisão de agir é minha."]),
    shot("P17", "Resultado da investigação: Observação, Hipótese e Lacunas",
         "Guardian, INVESTIGATOR, após a análise", "(1) bloco Observação, (2) bloco Hipótese, (3) bloco Lacunas")))

# ------------------------------------------------------------------ SECAO 5
parts.append(section("5. Perguntar aos dois RAG Assistants (3 a 4 minutos)",
                     "Use exatamente estas perguntas. Elas foram testadas contra o conteúdo real do projeto."))
parts.append(simple("O RAG Assistant só responde com base na documentação do projeto e mostra de onde tirou. Se a pergunta não tem nada a ver com a documentação, ele deve dizer que não sabe. Isso se chama <b>abstenção</b>. O corte é uma similaridade de 0.58: perguntas relevantes ficam entre 0.668 e 0.761; perguntas fora do assunto, entre 0.507 e 0.521."))
parts.append(step("5.1", "Gemini: pergunta com resposta na documentação",
    ...
    simple("Esperado: uma resposta que cita um documento real do projeto (similaridade em torno de 0.67) e liga ao erro que você acabou de gerar. <b>Clique na fonte citada</b> para mostrar que ela existe e não foi inventada."),
...
parts.append(step("5.2", "Gemini: pergunta fora do assunto (abstenção)",
    where("G", "Ainda no RAG Assistant (Gemini), pergunte:"),
    "<blockquote>Qual a receita de bolo de chocolate?</blockquote>",
    simple("Esperado: o sistema diz que não sabe (similaridade em torno de 0.52, abaixo do corte de 0.58). <b>Ele não deve inventar uma receita.</b>"),
...
parts.append(step("5.3", "Groq: mesma pergunta 1, outro provedor de IA",
...
# ------------------------------------------------------------------ SECAO 6
parts.append('<div class="pagebreak"></div>')
parts.append(section("6. Consertar o destino e mostrar a recuperação (2 minutos)"))
parts.append(step("6.1", "Destrancar a pasta",
    where("T", "No Terminal 1:"),
...
parts.append(step("6.2", "Mostrar que tráfego novo passa",
    ...
    where("G", "Espere cerca de 10 segundos e clique em <b>Monitor</b> no menu lateral esquerdo: os hosts voltam para <b>healthy</b>."),
    shot("P21", "Monitor de volta com os quatro hosts healthy",
         "Guardian, MONITOR", "(1) badges healthy nos quatro hosts")))
parts.append(step("6.3", "Explicar por que o INC-003 não some sozinho",
    ...
    where("P", "<b>Pelo Message Viewer</b> (mais fácil para gravar): Management Portal → <b>Interoperability</b> → <b>View</b> → <b>Messages</b> → selecione a mensagem do INC-003 com erro → clique em <b>Resend</b>."),
    shot("P22", "Message Viewer com a mensagem INC-003 selecionada e o botão Resend",
         "Management Portal → Interoperability → View → Messages", "(1) linha do INC-003, (2) botão Resend"),
...
# ------------------------------------------------------------------ SECAO 7
parts.append('<div class="pagebreak"></div>')
parts.append(section("7. Bônus implementados (2 a 3 minutos)"))
parts.append(step("7.1", "Business Rules: quem decide para onde vai cada incidente",
    ...
    shot("R1", "Caminho de cliques: Interoperability (1) → Build (2) → Business Rules (3)",
         "", "", "Print antigo: a Production aparece como Stopped no painel da direita; na gravação ela deve estar Running."),
    ...
    where("P", "No diálogo, dê dois cliques em <b>Business Rules</b> (ícone de pacote verde) → depois em <b>Rule</b> → depois em <b>IncidentRoutingRule</b>. Ou selecione o nome e clique em <b>OK</b>."),
    shot("P24", "Diálogo Open com o caminho Business Rules → Rule → IncidentRoutingRule",
         "Rule Editor, diálogo Open", "(1) pacote Business Rules, (2) pasta Rule, (3) IncidentRoutingRule, (4) botão OK"),
...
parts.append(step("7.2", "Busca híbrida (só falar, sem clicar)",
...
parts.append(step("7.3", "Segunda Production: PublicHealth (opcional)",
...
# ------------------------------------------------------------------ SECAO 8
parts.append(section("8. Metodologia com IA (1 minuto)"))
parts.append(step("8.1", "Mostrar a pasta de prompts",
...
# ------------------------------------------------------------------ SECAO 9
parts.append(section("9. Encerramento (30 a 45 segundos)"))
parts.append(step("9.1", "Falar o fechamento",
    where("G", "Deixe o Guardian aberto no <b>Monitor</b>, saudável."),
...
# ------------------------------------------------------------------ LIMPEZA + CHECKLIST
parts.append(section("Limpeza depois de gravar"))
parts.append(step("L.1", "Parar o loop", where("T", "No Terminal 2, pressione <b>Ctrl+C</b>. Se ele estiver em segundo plano, no Terminal 1: <code>pkill -f loop_trafego_saudavel</code> (ou <code>pkill -f \"durable/guardian/in\"</code> se você colou o bloco em vez de rodar o script).")))
parts.append(step("L.2", "Limpar as pastas",
    where("T", "No Terminal 1:"),
    cmd("docker exec -i iris-guardian sh -c 'D=/durable/guardian; rm -f $D/in/* $D/archive/* $D/out/* $D/out_priority/*'")))
parts.append(step("L.3", "Deixar a Production certa rodando",
...
parts.append(section("Checklist final antes de publicar"))
parts.append('''
<ul class="check">
...
# ------------------------------------------------------------------ ANEXO A
parts.append('<div class="pagebreak"></div>')
parts.append(section("Anexo A. Prints que ainda precisam ser capturados",
                     "Use como lista de tarefas quando estiver logado. Cada linha diz o que capturar e onde vai a bolinha vermelha."))
rows = "".join(f"<tr><td><b>{e(p)}</b></td><td>{e(t)}</td><td>{e(w)}</td><td>{e(m)}</td></tr>" for p, t, w, m in PENDING)
parts.append(f'<table class="t3 anexo"><tr><th>ID</th><th>O que capturar</th><th>Tela</th><th>Bolinha vermelha em</th></tr>{rows}</table>')
rr = "".join(f"<tr><td><b>{e(p)}</b></td><td>{e(t)}</td><td>{e(n)}</td></tr>" for p, t, n in REFUSED)
parts.append('<h3>Prints de referência já incluídos (trocar por prints atuais na versão final)</h3>')
parts.append(f'<table class="t3 anexo"><tr><th>ID</th><th>Tela</th><th>Por que é só referência</th></tr>{rr}</table>')
parts.append('<p>Já incluídos, reais: <b>R1</b> (caminho Interoperability → Build → Business Rules) e <b>R2</b> (regra RouteBySeverity carregada).</p>')
...
pre { background:#0d1444; color:#e9efff; padding:9px 11px; border-radius:6px; font-family:"DejaVu Sans Mono",monospace; font-size:7.6pt; line-height:1.4; white-space:pre-wrap; word-break:break-all; margin:3px 0 8px; page-break-inside:avoid; }
...
.cap.ref { color:#8a5a00; }
h3 { color:#0A1965; font-size:11.5pt; margin:14px 0 4px; }
'''
doc = f'<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Roteiro de Gravação V3</title><style>{CSS}</style></head><body>{"".join(parts)}</body></html>'
...
with sync_playwright() as p:
    ...
    pg.pdf(path=OUT_PDF, format="A4", print_background=True,
           ...
           footer_template='<div style="font-size:8px;color:#777;width:100%;text-align:center;">IRIS Production Guardian · Roteiro de Gravação V3 · <span class="pageNumber"></span>/<span class="totalPages"></span></div>',
           margin={"top": "14mm", "bottom": "16mm", "left": "13mm", "right": "13mm"})
    b.close()
print("pendentes:", len(PENDING))
for x in PENDING:
    print(x[0], "-", x[1])
