// Gera 08_ROTEIRO_GRAVACAO_VIDEO_V5.pdf com prints reais capturados ao vivo
...
parts.push(warn(
  ...
  "Esta revisão também corrigiu 3 pontos que só apareceram testando ao vivo (herdados da V4): (a) o print do erro #5005 exige dois cliques a mais do que o documentado antes — abrir a aba <b>Trace</b> da mensagem, clicar em <b>View Full Trace</b> e depois no losango vermelho de erro; " +
  ...
  "(c) o diálogo \"Open\" do Rule Editor tem 3 níveis reais — <b>Guardian → Rule → IncidentRoutingRule</b> — e não \"Business Rules → Rule → IncidentRoutingRule\" como versões anteriores descreviam."
));
...
const doc = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Roteiro de Gravação V5</title><style>${CSS}</style></head><body>${parts.join("")}</body></html>`;
...
(async () => {
  ...
  await pg.pdf({
    ...
    footerTemplate: '<div style="font-size:8px;color:#777;width:100%;text-align:center;">IRIS Production Guardian · Roteiro de Gravação V5 · <span class="pageNumber"></span>/<span class="totalPages"></span></div>',
