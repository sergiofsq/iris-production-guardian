// Gera 08_ROTEIRO_GRAVACAO_VIDEO_V8.pdf com prints reais capturados ao vivo
// (todos os 25 + R1/R2, incluindo P01/P25 capturados manualmente pelo dono).
const fs = require("fs");
const path = require("path");
const { chromium } = require("playwright");

const SHOTS_DIR = "/Users/sergiofernandes/Projetos/IRIS Production Guardian/docs/planejamento/screenshots";
const APP_DIR = "/Users/sergiofernandes/Projetos/IRIS Production Guardian/Imagens/Aplicacao";
...
// P01 e P25 exigem a tela física do Mac (mesa de gravação, Finder) - capturados
// manualmente pelo dono do projeto com Cmd+Shift+4 e adicionados em Imagens/Aplicacao/.
for (const id of ["P01", "P25"]) {
  const p = path.join(APP_DIR, id + ".jpg");
  if (fs.existsSync(p)) REAL[id] = p;
}
...
<div class="cover">
  ...
  <h1>Roteiro de Gravação V8</h1>
  <div class="tagline">Passo a passo com prints reais, feito para quem nunca trabalhou com interoperabilidade</div>
  <div class="meta">Versão de 19/09/2026 · Todos os 25 prints + R1/R2 são reais · Login da aplicação e do Management Portal: <b>guardian / guardian</b></div>
</div>
`);

parts.push(warn(
  "<b>Estado deste arquivo (leia primeiro):</b> versão definitiva — <b>todos os 25 prints (mais R1/R2) são reais</b>, sem nenhum quadro \"PRINT PENDENTE\" restante. " +
  "23 foram capturados ao vivo pelo Claude contra a aplicação e o Management Portal rodando de verdade (Playwright, sem precisar de tela física); " +
  "<b>P01</b> (mesa de gravação) e <b>P25</b> (pasta <code>entregaveis/Prompts</code> no Finder) foram capturados manualmente pelo dono do projeto, já que são a tela física do Mac. " +
  "Esta revisão corrigiu 9 pontos que só apareceram testando ao vivo (a) a (d) herdados das versões anteriores, (e) a (i) novos — ver abaixo: (a) o print do erro #5005 exige dois cliques a mais do que o documentado antes — abrir a aba <b>Trace</b> da mensagem, clicar em <b>View Full Trace</b> e depois no losango vermelho de erro; " +
  "(b) o diálogo \"Open\" do Rule Editor tem 3 níveis reais — <b>Guardian → Rule → IncidentRoutingRule</b> — e não \"Business Rules → Rule → IncidentRoutingRule\" como versões anteriores descreviam; " +
  "(c) <b>[V6]</b> um bug de unidade real foi corrigido no código: a janela de \"erro recente\" que decide o status <code>degraded</code> (<code>Guardian.Monitor.StatusCollector::ErrorWindowSeconds</code>) estava em 900s (15 minutos) — o comentário original dizia que devia bater com o <code>FailureTimeout</code> real do framework, que é <b>15 segundos</b>, não 15 minutos. Corrigido para <code>15</code> e confirmado ao vivo: a janela de erro recente ficou em 15 segundos (a volta a <code>healthy</code> foi depois refinada no item (f)); " +
  "(d) <b>[V7]</b> a mensagem de recuperação da seção 6.2 estava com severidade <code>LOW</code>, que a regra <code>RouteBySeverity</code> nunca manda para <code>out_priority</code> (o destino que quebramos e consertamos) — ela ia para <code>out</code>, que nunca esteve bloqueado, então não provava nada sobre a correção. Corrigido para <code>CRITICAL</code> (mesma severidade do <code>INC-003</code>) e confirmado ao vivo: agora o arquivo aparece de verdade em <code>out_priority</code> depois do <code>chmod 755</code>; " +
  "(e) <b>[novo nesta versão]</b> <code>chmod 555</code> na pasta <b>não bloqueia</b> a regravação de um arquivo que <b>já existe</b> dentro dela (a permissão da pasta só impede criar, apagar e renomear entradas; sobrescrever um arquivo existente depende só da permissão do próprio arquivo). Como o destino grava sempre em <code>incident_&lt;ID&gt;.txt</code>, um <code>incident_INC-003.txt</code> deixado por uma gravação anterior fazia a demo <b>não</b> degradar. O passo 3.1 agora apaga esse arquivo antes de bloquear a pasta; e o comando de limpeza (0.4 e L.2) agora faz <code>chmod 755</code> <b>antes</b> do <code>rm</code> — com a pasta em 555 o <code>rm</code> falhava e o <code>chmod</code> encadeado por <code>&amp;&amp;</code> nunca rodava; " +
  "(f) <b>[novo nesta versão]</b> regra do Monitor alterada: <code>degraded</code> agora também vale para <b>erro não recuperado</b> — o host continua degradado enquanto o último erro dele não for seguido de uma mensagem concluída com sucesso no mesmo caminho. Antes o estado voltava a <code>healthy</code> sozinho depois de 15 segundos mesmo com o destino ainda quebrado. Consequência para a gravação: depois do <code>chmod 755</code> os hosts só voltam a <code>healthy</code> quando a mensagem do passo 6.2 chegar com sucesso — não é mais preciso esperar 15 segundos; " +
  "(g) <b>[novo nesta versão]</b> o Investigator agora tem <b>fallback automático para o Groq</b>: se o Gemini devolver erro (por exemplo HTTP 503 por alta demanda), a análise é gerada pelo Groq com o mesmo prompt, sem o usuário precisar tentar de novo; " +
  "(h) <b>[novo nesta versão]</b> <b>tudo o que aparece na tela está em inglês</b>: os prints P02–P08, P12, P15–P21 foram recapturados ao vivo com a interface em EN; o corpus do RAG Assistant foi traduzido para inglês (<code>docs/corpus_en/</code>, reindexado no lugar, mantendo os IDs dos documentos), então as fontes citadas nos prints P17–P20 aparecem em inglês; as perguntas dos passos 5.1–5.3 agora são em inglês (<i>What is the cause of error #5005?</i> / <i>What is the recipe for chocolate cake?</i>); a saída do script do loop e as mensagens de incidente dos passos 3.2 e 6.2 também foram para inglês, e o script foi renomeado para <code>healthy_traffic_loop.sh</code>; "  +
  "(i) <b>[pendente, é com você]</b> <b>P01</b> (mesa de gravação) precisa ser recapturado com o Management Portal em inglês, e <b>P25</b> (Finder) precisa ser recapturado depois do rename dos arquivos de <code>entregaveis/Prompts/</code> para inglês — os dois são prints da tela física do Mac e os arquivos atuais ainda mostram o idioma antigo."
));
...
<tr><td><b>Healthy / Degraded / Unavailable</b></td><td>Saudável (verde), degradado (amarelo: funciona com problema) e indisponível (vermelho: fora do ar). Um host fica <code>degraded</code> enquanto houver fila pendente, um erro nos últimos <b>15 segundos</b> (a mesma janela que o framework usa para desistir de uma tentativa, o <code>FailureTimeout</code>) ou um <b>erro ainda não recuperado</b>: o último erro dele não foi seguido de uma mensagem concluída com sucesso no mesmo caminho. Ele volta a <code>healthy</code> quando uma mensagem nova passa com sucesso.</td></tr>
...
write "Name: ",tName," State: ",tState,!
halt
EOF`, "Bloco colado de uma vez (várias linhas de propósito)"),
    simple("Você deve ver <code>State: 1</code>. O número 1 significa Running (rodando)."),
...
halt
EOF`, "Bloco colado de uma vez (várias linhas de propósito)"),
    shot("P02", "Terminal 1 mostrando o resultado do comando: State: 1",
      "Terminal 1 do Mac após colar o bloco de conferência", "(1) linha \"Name: Guardian.Production.GuardianProduction State: 1\"")),
  step("0.4", "Limpar as pastas de mensagens",
    where("T", "No Terminal 1:"),
    cmd("docker exec -i iris-guardian sh -c 'D=/durable/guardian; chmod 755 $D/out $D/out_priority && rm -f $D/in/* $D/archive/* $D/out/* $D/out_priority/*'"),
    simple("Apaga os arquivos de testes anteriores para o vídeo começar do zero e garante que as pastas de saída aceitam gravação. O <code>chmod</code> vem <b>antes</b> do <code>rm</code> de propósito: se a pasta <code>out_priority</code> ficou em 555 da gravação anterior, o <code>rm</code> falharia com \"Permission denied\".")),
...
while true; do
  ...
  "<p><b>Alternativa mais segura</b> (mesmo resultado, sem risco de colagem quebrada), no Terminal 2, dentro da pasta do projeto:</p>" + cmd("sh docs/experiments/healthy_traffic_loop.sh", "Comando único") +
  "<p>Para mudar o intervalo para 10 segundos: <code>sh docs/experiments/healthy_traffic_loop.sh 10</code>.</p>",
  simple('Cada arquivo criado na pasta <code>in</code> é uma mensagem entrando na linha de montagem. Você vai ver no Terminal 2 uma linha nova a cada 6 segundos dizendo "sent LOOP-...". Deixe rodando <b>até a limpeza, no final</b>; só pare com Ctrl+C lá.'),
  shot("P06", 'Terminal 2 com o loop rodando, linhas "sent LOOP-..."',
    "Terminal 2 do Mac", '(1) linhas "[hh:mm:ss] sent LOOP-..." aparecendo a cada 6 segundos')));
...
parts.push(step("3.1", "Bloquear o destino prioritário",
  where("T", "No Terminal 1:"),
  cmd("docker exec -i iris-guardian sh -c 'D=/durable/guardian/out_priority; rm -f $D/incident_INC-003.txt && chmod 555 $D'"),
  warn("<b>Achado ao vivo:</b> <code>chmod 555</code> na pasta <b>não</b> impede regravar um arquivo que <b>já existe</b> dentro dela. Se um <code>incident_INC-003.txt</code> de uma tentativa anterior ainda estiver lá, o IRIS simplesmente sobrescreve e o Monitor fica verde. Por isso o comando acima apaga esse arquivo <b>antes</b> de bloquear — assim o IRIS precisa <i>criar</i> o arquivo e falha de verdade."),
  simple("Duas coisas em um comando só: (a) remove o <code>incident_INC-003.txt</code> de uma gravação anterior, se existir; (b) <code>chmod 555</code> tira a permissão de gravar na pasta <code>out_priority</code>. É como trancar a porta do destino. A pasta continua existindo, mas ninguém consegue escrever nela. Isso simula um disco cheio, uma permissão negada ou uma rede fora do ar. Não mexemos em nada dentro do IRIS."),
...
parts.push(step("3.2", "Disparar um incidente grave",
  ...
  cmd(`docker exec -i iris-guardian sh -c 'F=/durable/guardian/in/demo3.txt; echo "INC-003|IRIS.Service.PaymentGateway|CRITICAL|Simulated write failure at the destination" > $F && chown irisowner:irisowner $F'`),
...
parts.push(step("4.2", "Rodar a investigação",
  ...
  say(["A IA está analisando agora, em tempo real, os dados reais da falha. Se o provedor principal estiver sobrecarregado, o sistema troca sozinho para o segundo provedor, sem eu precisar refazer nada."]),
  ...
  warn("Se o Gemini devolver um erro transitório de sobrecarga (HTTP 503), o Investigator usa o <b>Groq automaticamente</b> (fallback) e entrega a análise mesmo assim — você não precisa fazer nada. Isso realmente aconteceu na preparação desta revisão. Se os dois provedores falharem, a tela mostra a evidência coletada e uma mensagem honesta de indisponibilidade em vez de inventar uma análise.")));
parts.push(step("4.3", "Ler o resultado para a câmera",
  ...
  "<p><b>Nota sobre os eventos:</b> a evidência lista as mensagens em que o componente escolhido é <b>origem ou destino</b>. Por isso o <code>IncidentRouterProcess</code> também aparece — ele é o outro lado de cada mensagem enviada ao <code>PriorityOutputOperation</code> (requisição) e recebida dele (resposta). Não há eventos de componentes não envolvidos.</p>",
...
parts.push(step("5.1", "Gemini: pergunta com resposta na documentação",
  ...
  "<blockquote>What is the cause of error #5005?</blockquote>",
...
parts.push(step("5.2", "Gemini: pergunta fora do assunto (abstenção)",
  ...
  "<blockquote>What is the recipe for chocolate cake?</blockquote>",
...
parts.push(step("5.3", "Groq: mesma pergunta 1, outro provedor de IA",
  where("G", "Clique em <b>RAG Assistant (Groq)</b> no menu lateral esquerdo e repita: <i>What is the cause of error #5005?</i>"),
...
parts.push(step("6.2", "Mostrar que o destino consertado realmente aceita escrita de novo",
  warn("<b>Achado ao vivo nesta revisão:</b> a mensagem de recuperação precisa ter a <b>mesma severidade</b> que falhou (<code>CRITICAL</code>/<code>HIGH</code>/<code>MEDIUM</code>), não <code>LOW</code>. A regra <code>RouteBySeverity</code> só manda para <code>out_priority</code> quando a severidade é <code>HIGH</code>, <code>CRITICAL</code> ou <code>MEDIUM</code> — uma mensagem <code>LOW</code> vai para <code>out</code>, a pasta que nunca foi bloqueada, e não prova nada sobre a correção que acabamos de fazer."),
  where("T", "No Terminal 1. Comando único, uma linha só:"),
  cmd(`docker exec -i iris-guardian sh -c 'F=/durable/guardian/in/demo4.txt; echo "INC-004|IRIS.Service.PaymentGateway|CRITICAL|Critical flow after destination recovery" > $F && chown irisowner:irisowner $F'`),
  simple("Mesmo formato do passo 3.2, mas agora com severidade <code>CRITICAL</code> — a mesma do <code>INC-003</code> — para forçar a mensagem a passar de novo por <code>Guardian.Operation.PriorityOutputOperation</code>, exatamente o destino que consertamos no passo 6.1. Só assim o teste prova algo: se essa mensagem for gravada em <code>out_priority</code>, a correção funcionou de verdade."),
  where("T", "Confirme que o arquivo foi escrito no destino certo (comando único):"),
  cmd("docker exec -i iris-guardian ls -la /durable/guardian/out_priority", "Comando único"),
  simple("Deve aparecer <code>incident_INC-004.txt</code> lá dentro, escrito com sucesso — diferente do <code>INC-003</code> na seção 3, que nunca chegou a ser criado porque a pasta estava bloqueada naquele momento."),
  where("G", "Clique em <b>Monitor</b> no menu lateral esquerdo (não é preciso esperar 15 segundos)."),
  simple("O <code>INC-004</code> foi gravado com sucesso na hora. Como <code>degraded</code> inclui <b>erro não recuperado</b> (o último erro do host ainda não foi seguido de um sucesso no mesmo caminho), os hosts que estavam amarelos voltam a <code>healthy</code> <b>exatamente</b> quando esta mensagem passa com sucesso pelo <code>PriorityOutputOperation</code> — sem depender de tempo. Se você <b>não</b> enviar esta mensagem, o Monitor continua amarelo mesmo com a pasta já destrancada, e isso é o comportamento esperado."),
  say(["O sistema aceita e processa o tráfego crítico imediatamente, sem reiniciar nada. Mandei de propósito a mesma severidade que falhou antes, para provar que o destino realmente voltou a aceitar escrita. E repare no indicador de saúde: ele não voltou a verde por passar o tempo, e sim porque uma mensagem nova concluiu com sucesso exatamente no caminho que estava com erro."]),
  shot("P21", "Monitor após a recuperação: os quatro hosts de volta em healthy",
    "Guardian, MONITOR", "(1) badges healthy nos quatro hosts")));
...
parts.push(step("7.1", "Business Rules: quem decide para onde vai cada incidente",
  ...
  say([
    ...
    "O ponto importante aqui é que essa decisão de roteamento é configurável visualmente, sem recompilar nada. Se eu quisesse adicionar uma quarta severidade ou trocar o destino, eu editaria essa regra aqui e recompilaria só ela, sem tocar no código dos outros componentes. É exatamente essa regra que decidiu, nas seções 3 e 6, mandar tanto o INC-003 quanto o INC-004 para o mesmo destino prioritário — os dois são CRITICAL. A diferença entre um falhar e o outro funcionar não veio da regra, veio do estado da pasta de destino em cada momento."])));
...
parts.push(step("8.1", "Mostrar a pasta de prompts",
  where("T", "No Finder (ou no editor de código), abra a pasta <code>entregaveis/Prompts/</code> do projeto. São 12 arquivos com a sequência real de prompts por fase, com os erros reais encontrados e corrigidos. Você <b>não</b> precisa ler nada; só aponte que existe e abra um arquivo rapidamente como exemplo."),
  shot("P25", "Pasta entregaveis/Prompts com os 12 arquivos",
...
parts.push(step("L.1", "Parar o loop", where("T", 'No Terminal 2, pressione <b>Ctrl+C</b>. Se ele estiver em segundo plano, no Terminal 1: <code>pkill -f healthy_traffic_loop</code> (ou <code>pkill -f "durable/guardian/in"</code> se você colou o bloco em vez de rodar o script).')));
parts.push(step("L.2", "Limpar as pastas",
  where("T", "No Terminal 1:"),
  cmd("docker exec -i iris-guardian sh -c 'D=/durable/guardian; chmod 755 $D/out $D/out_priority && rm -f $D/in/* $D/archive/* $D/out/* $D/out_priority/*'"),
  simple("O <code>chmod 755</code> vem antes do <code>rm</code>: se a pasta <code>out_priority</code> ainda estiver em 555, o <code>rm</code> falha com \"Permission denied\".")));
...
const doc = `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Roteiro de Gravação V8</title><style>${CSS}</style></head><body>${parts.join("")}</body></html>`;
...
(async () => {
  ...
  await pg.pdf({
    ...
    footerTemplate: '<div style="font-size:8px;color:#777;width:100%;text-align:center;">IRIS Production Guardian · Roteiro de Gravação V8 · <span class="pageNumber"></span>/<span class="totalPages"></span></div>',
